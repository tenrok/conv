...
// GetField get value from struct field
func GetField(structd interface{}, name string) (interface{}, error) {
	structValue := reflect.ValueOf(structd)
	if IsPtr(structd) {
	...
	if !fieldValue.IsValid() {
		return nil, fmt.Errorf("No such field: %s in structPtr", name)
	}
